...
jobs:
  ...
  upload-to-ftp:
    ...
    steps:
      ...
      - name: Open All Ports on VPS (if needed)
        run: |
          echo "Opening all ports to allow Active Mode FTP..."
          sudo iptables -P INPUT ACCEPT
          sudo iptables -P OUTPUT ACCEPT
          sudo iptables -P FORWARD ACCEPT
          sudo iptables -F
          sudo ufw disable || echo "UFW not enabled"

      - name: Upload to FTP using Active Mode
        run: |
          echo "Uploading build using Active Mode FTP..."
          curl --verbose --ftp-port - -T develop-${{ github.event.inputs.BUILD_VERSION }}.zip -u ${{ secrets.FTP_USERNAME }}:${{ secrets.FTP_PASSWORD }} ftp://${{ secrets.FTP_HOST }}:${{ secrets.FTP_PORT }}/
