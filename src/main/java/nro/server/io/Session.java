...
//        } else {
//            Service.getInstance().sendThongBaoOK(this, "Server hiện tại đang bảo trì cư dân không thể vào server vào thời điểm hiện tại");
//        }
    }
...
//            RewardService.gI().rewardFirstTimeLoginPerDay(player);
            ...
            this.sendThongBaoTop1ZoGame(player);
            if (player.isPl()) {
                if (player.inventory.itemsBody.size() > 14) {
                    Item it = player.inventory.itemsBody.get(14);
    ...
    public void CheckTop(Player player) {
        ...
        if (!Util.isNullOrEmpty(topPlayer)) {
            if (Objects.equals(player.name, topPlayer)) {
                Service.getInstance().sendThongBaoAllPlayer("[THÔNG BÁO] Top 1 Đại Gia tên " + player.name + " vừa đăng nhập vào game");
            }
        }
    }

    private void sendThongBaoTop1ZoGame(Player player) {
        if (player.name.equals("brolysuper") || player.id == 54) {
            Service.getInstance().sendThongBaoAllPlayer("[THÔNG BÁO] Top 1 Đại Gia tên " + player.name + " vừa đăng nhập vào game");
        }
    }
}
