    ...
    public static void GetRewardDay(Player player) {
        ...
        if (rank > 0 && rank <= 10) {
            if (CanGetRewardDay(player.id)) {
                ...
                Manager.addPlayerRubyHistory(player.id, oldRuby, player.inventory.ruby, "SieuHangManager-GetRewardDay");
            ...
            // ekko bỏ thông báo top 1 vô game
//            if (rank == 1) {
...
//    }
    ...
    public static void CreateClone(Player player) {
        ...
        try (Connection con = DBService.gI().getConnectionForGetPlayer();) {
            ...
            ps.setLong(8, player.nPoint.def);
