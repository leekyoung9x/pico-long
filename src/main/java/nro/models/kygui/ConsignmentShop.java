...
//            }
    ...
    private void consign(Player player, short itemID, byte monneyType, int money, int quantity) {
        if (player.playerTask.taskMain.id < 22) {
            Service.getInstance().sendThongBao(player, "Bạn phải hoàn Đánh bại Tiểu đội Fide mới có thể sửa dụng ký gửi");
            show(player);
            return;
        }
        if (player.nPoint.power < 40_000_000_000L) {
            Service.getInstance().sendThongBao(player, "Bạn chưa đủ 40 tỷ sức mạnh");
            show(player);
            return;
        }
//        if (!player.getSession().actived) {
...
//        }
        ...
        if (monneyType == 0) {
            Service.getInstance().sendThongBao(player, "Ký gửi vàng chúng tôi không nhận");
            show(player);
            return;
        } else {
...
//                }
                if (player.playerTask.taskMain.id < 22) {
                    Service.getInstance().sendThongBao(player, "Bạn phải hoàn thành nhiệm vụ Fide Đại Ke");
                    show(player);
                    return;
                }
                if (player.nPoint.power < 40_000_000_000L) {
...
//                    player.inventory.subGold(money);
                    ...
                    if (inventory.gem < money) {
                        NpcService.gI().createTutorial(player, -1, "Bạn không đủ ngọc xanh");
                        return;
                    }
                    player.inventory.subGem(money);
                }
        ...
        for (ConsignmentItem item : list) {
            if (item.template.id == itemID && item.getConsignorID() == player.id && item.isSold()) {
                if (item.getPriceGold() > 0) {
                    ...
                    player.inventory.addGem(item.getPriceGem() - (item.getPriceGem() * 10 / 100));
                }
                removeItem(item);
                Service.getInstance().sendMoney(player);
