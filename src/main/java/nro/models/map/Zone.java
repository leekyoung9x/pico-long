...
public class Zone {
    ...
    public List<Player> getMiniPet() {
        return this.minipets;
    }
            ...
            synchronized (itemMap) {
                ...
                if (!itemMap.isPickedUp) {
                    if (itemMap.playerId == player.id || itemMap.playerId == -1) {
                        ...
                        int maxQuantity = 0;
                        if (ItemService.gI().isItemNoLimitQuantity(item.template.id) || item.isSpl()) {
                            maxQuantity = 999_999;
                        }
    ...
    public void infoPlayer(Player plReceive, Player plInfo) {
        ...
        try {
            ...
            if (plInfo.clan != null) {
                ...
                name = "[" + Util.RemoveTagSize(plInfo.clan.name) + "]" + plInfo.name;
            ...
            } else if (plInfo.isPet && ((Pet) plInfo).typePet == ConstPet.WHIS || plInfo.isPet && ((Pet) plInfo).typePet == ConstPet.MABU_NHI || plInfo.isPet && ((Pet) plInfo).typePet == ConstPet.BILL_CON || plInfo.isPet && ((Pet) plInfo).typePet == ConstPet.ZENO || plInfo.isPet && ((Pet) plInfo).typePet == ConstPet.BLACK_GOKU) {
                msg.writer().writeInt(-1);
