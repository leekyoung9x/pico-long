...
public class DoanhTrai {
    ...
    public static final int DATE_WAIT_FROM_JOIN_CLAN = 2;
    ...
    private List<Integer> playerHaveGotRewards = new ArrayList<Integer>();
...
//        }
        ...
        ChangeMapService.gI().changeMap(plOpen, 53, -1, 35, 432);

        sendTextDoanhTraiAllClan();
    }

    public void openDoanhTrai(Player plOpen, Clan clan, boolean isTeam) {
        this.lastTimeOpen = System.currentTimeMillis();
        this.isOpened = true;
        this.clan = clan;
        this.clan.timeOpenDoanhTrai = this.lastTimeOpen;
...
//            }
    ...
    public boolean isClearDoanhTrai() {
        boolean result = true;
    ...
    public boolean checkPlayerHaveGotReward(int playerId) {
        return !playerHaveGotRewards.contains(playerId);
    }

    public boolean addPlayerHaveGotReward(int playerId) {
        boolean result = !playerHaveGotRewards.contains(playerId);

        if (result) {
            playerHaveGotRewards.add(playerId);
        }
