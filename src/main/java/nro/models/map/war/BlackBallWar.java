...
public class BlackBallWar {
    ...
    public static final int COST_X3 = 20000;
    public static final int COST_X5 = 20000;
    public static final int COST_X7 = 20000;
...
//    public static final byte SECOND_CLOSE = 0;
    ...
    private void win(Player player) {
        ...
        if (player.clan != null) {
            ...
            Service.getInstance().sendThongBao(player, "Chúc mừng bang hội của bạn đã "
                    + "dành chiến thắng ngọc rồng sao đen " + star + " sao");
            if (star == 1) {
                getRewardItem(player);
            } else if(star == 2) {
                getRewardItemTwoStar(player);
            }
        }
    ...
    private void getRewardItem(Player player) {
        Item nr = ItemService.gI().createNewItem((short) SummonDragon.NGOC_RONG_NOEL[1], 1);
        InventoryService.gI().addItemBag(player, nr, 999);
        InventoryService.gI().sendItemBags(player);
        Service.getInstance().sendThongBao(player, "Bạn nhận được " + nr.template.name);
    }

    private void getRewardItemTwoStar(Player player) {
        Item nr = ItemService.gI().createNewItem((short) SummonDragon.NGOC_RONG_NOEL[2], 1);
        InventoryService.gI().addItemBag(player, nr, 999);
        InventoryService.gI().sendItemBags(player);
        Service.getInstance().sendThongBao(player, "Bạn nhận được " + nr.template.name);
    }
    ...
    private void kickOutOfMap(Player player) {
        player.isPhu = false;
        player.zone.finishBlackBallWar = false;
        player.zone.isPickItem = true;
        if (player.cFlag == 8) {
            Service.getInstance().changeFlag(player, Util.nextInt(1, 7));
        }
    ...
    public void xHPKI(Player player, byte x) {
        if (player.isPhu) {
            Service.getInstance().sendThongBao(player, "Ban da phu roi");
            return;
        }
        int cost = 0;
        switch (x) {
        ...
        if (player.inventory.gem >= cost) {
            ...
            Service.getInstance().sendThongBao(player, "Không đủ ngọc xanh để thực hiện, còn thiếu "
                    + Util.numberToMoney(cost - player.inventory.gem) + " ngọc xanh");
        }
    }
