...
//                }
    ...
    private void Finish(Player plAtt) {
        if (plAtt.zone != null) {
            int quantity = Util.nextInt(1, 99);
            Item bk = ItemService.gI().createNewItem((short) 1229, quantity);
            InventoryService.gI().addItemBag(plAtt, bk, 999999);
            Service.getInstance().sendThongBao(plAtt, "Bạn đã nhận được " + bk.template.name + " x" + quantity);
