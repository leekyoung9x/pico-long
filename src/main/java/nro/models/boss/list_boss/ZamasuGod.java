...
public class ZamasuGod extends Boss {

    public ZamasuGod() {
        super(BossFactory.ZAMASU, BossData.ZAMASU_GOD);
    }

    protected ZamasuGod(byte id, BossData bossData) {
        super(id, bossData);
    }
...
//                Item phaohoa = ItemService.gI().createNewItem((short) 2086);
//                int quantity = Util.nextInt(1, 20);
//                phaohoa.quantity = quantity;
//                InventoryService.gI().addItemBag(plKill, phaohoa, 9999);
//                InventoryService.gI().sendItemBags(plKill);
//                Service.getInstance().sendThongBao(plKill, "Bạn nhận được x" + quantity + " " + phaohoa.template.name);
                Service.getInstance().sendThongBao(plKill, "Bạn nhận được 1 cái nịt");
            }
        }
    }
