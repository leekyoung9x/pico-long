...
public class BossManager {
    ...
    public void showListBoss(Player player) {
        Message msg;
        try {
            // ekko không hiển thị boss ở mabu war
            long aliveBossCount = BOSSES_IN_GAME.stream().filter(boss -> boss != null && !BossFactory.isYar((byte) boss.id) && !BossFactory.isDoanhTrai(boss) && !BossFactory.isMabuWar(boss)).count();
            ...
            for (int i = 0; i < BOSSES_IN_GAME.size(); i++) {
                ...
                // ekko không hiển thị boss ở mabu war
                if (BossFactory.isYar((byte) boss.id) || BossFactory.isDoanhTrai(boss) || BossFactory.isMabuWar(boss)) {
                    continue;
                }
    ...
    public void showListBossTuanLoc(Player player) {
        Message msg;
        try {
            long aliveBossCount = BOSSES_IN_GAME.stream().filter(boss -> boss != null && BossFactory.isTuanLoc(boss)).count();
            int count = (int) (aliveBossCount > Byte.MAX_VALUE ? Byte.MAX_VALUE : aliveBossCount);
            msg = new Message(-96);
            msg.writer().writeByte(0);
            msg.writer().writeUTF("Boss (SL: " + count + ")");
    ...
    // Bắt buộc tạo 1 boss và xuất hiện ngay lập tức.
    public static void forceCreateBoss(int bossId){
        try{
            Boss boss = BossFactory.createBoss(bossId);
            boss.changeStatus(Boss.JUST_RESPAWN);
            Thread.sleep(1000);
            boss.changeStatus(Boss.JUST_JOIN_MAP);
            Thread.sleep(1000);
        }
        catch (Exception e){
            e.printStackTrace();
