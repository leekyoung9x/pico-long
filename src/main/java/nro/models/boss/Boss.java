...
public abstract class Boss extends Player implements BossInterface {
    ...
    public static final byte JUST_RESPAWN = 75; // khởi tạo lại, rồi chuyển sang nghỉ
    ...
    public void init() {
        ...
        if (arrHp.length == 1) {
            ...
            hp = Util.nextLong(arrHp[0], arrHp[1]);
        }
        switch (this.typeHp) {
            case HP_NORMAL:
                this.nPoint.hpg = hp;
        ...
        try {
            ...
            if (Util.canDoWithTime(lastTimeUpdate, 60000)) {
                if (timeAppear >= TIME_RESEND_LOCATION) {
                    if (this.zone != null && !(this instanceof BossMabuWar) && !(this instanceof BossYardart) && !(this instanceof BossDoanhTrai) && !(this instanceof BossBanDoKhoBau)) {
    ...
    public void nextMabu(boolean isDie) {
        ...
        switch (id) {
            case BossFactory.MABU_MAP:// boss die là bư mập => Summon Super Bư
                ...
                this.changeStatus(RESPAWN);
                break;
            case BossFactory.SUPER_BU:// boss die là Super Bư => Bư Tenk
                this.leaveMap();
            ...
            case BossFactory.BU_TENK:// boss die là Bư Tenk => Summon bư Han
                ...
                this.changeStatus(RESPAWN);
                break;
            case BossFactory.BU_HAN:// boss die là bư han => Kid bư
                this.leaveMap();
                this.id = BossFactory.KID_BU;
                this.data = BossData.KID_BU;
                this.changeStatus(RESPAWN);
                break;
            case BossFactory.KID_BU:// boss die là Kid bư => MABU_MAP
                this.leaveMap();
                this.id = BossFactory.MABU_MAP;
                this.data = BossData.MABU_MAP;
                this.changeStatus(RESPAWN);
                break;
            default:
                if (isDie) {
...
//            if (this.zone != null) {
            if (this.zone != null && !(this instanceof BossMabuWar) && !(this instanceof BossYardart) && !(this instanceof BossDoanhTrai) && !(this instanceof BossBanDoKhoBau)) {
                ...
                ServerNotify.gI().notify("Boss " + this.name + " vừa xuất hiện tại " + this.zone.map.mapName + " zone: " + this.zone.zoneId);
//                System.out.println("Boss: " + this.name + " xuất hiện mapId: " + this.zone.map.mapId + " zone: " + this.zone.zoneId);
    ...
    public void dropItemReward(int tempId, int playerId, int... quantity) {
        if (!this.zone.map.isMapOffline && (this.zone.map.type == ConstMap.MAP_NORMAL || this.zone.map.type == ConstMap.MAP_DOANH_TRAI)) {
            ...
            ItemMap itemMap = new ItemMap(this.zone, tempId, (quantity != null && quantity.length == 1)
                    ? quantity[0] : 1, x, y, playerId);
