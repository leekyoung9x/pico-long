...
public class EffectSkin {
    ...
    private long lastTimeDepTraiQua;
    private long lastTimeGaiXink;
    ...
    public boolean isHoaXuong;
    public long lastTimeHoaDa;
    public long lastTimeHoaXuong;
    public long lastTimeUseHoaXuong;
    public int timeHoaDa;
    public int timeHoaXuong;
    ...
    public void update() {
        ...
        if (isHoaXuong && (Util.canDoWithTime(lastTimeHoaXuong, timeHoaXuong))) {
            EffSkinService.gI().removeHoaGay(this.player);
        }
    ...
    private void updateChatPlayer() {
        try {
            if ((this.player.id == 54 || this.player.name.equals("admin") || this.player.id == 7636 || this.player.id == 8761|| this.player.id ==  391)
                    //                    && this.player.zone != null
            ...
            // trang bị item vòng thiên sứ thì sau 30s player trong khu vực chat
            if(this.player != null && !this.player.isPet && !this.player.isBoss && this.player.inventory != null && this.player.inventory.itemsBody != null && this.player.inventory.itemsBody.size() >= 12 && Util.canDoWithTime(lastTimeDepTraiQua, 30_000)) {
                Item flagBag = this.player.inventory.itemsBody.get(12);
                if(flagBag.isNotNullItem() && flagBag.template.id == ConstItem.VONG_THIEN_SU) {
                    List<Player> players = new ArrayList<>();
                    List<Player> playersMap = this.player.zone.getNotBosses();
                    for (int i = 0; i < playersMap.size(); i++) {
                        Player pl = playersMap.get(i);
                    ...
                    for (int i = 0; i < players.size(); i++) {
                        Player pl = players.get(i);
                        if (!pl.isMiniPet && !pl.isPet && this.player.id != pl.id) {
                            Service.getInstance().chat(pl, "Ôi anh này đẹp trai quá");
                        }
                    }
                }
                this.lastTimeDepTraiQua = System.currentTimeMillis();
            }

            // trang bị có option gái xink thì sau 30s player trong khu vực chat
            if(this.player != null && !this.player.isPet && !this.player.isBoss && this.player.inventory != null && this.player.inventory.itemsBody != null && Util.canDoWithTime(lastTimeGaiXink, 30_000)) {
                if(this.player.nPoint.isGaiXink) {
                    List<Player> players = new ArrayList<>();
                    List<Player> playersMap = this.player.zone.getNotBosses();
                    for (int i = 0; i < playersMap.size(); i++) {
                        Player pl = playersMap.get(i);
                    ...
                    for (int i = 0; i < players.size(); i++) {
                        Player pl = players.get(i);
                        if (!pl.isMiniPet && !pl.isPet && this.player.id != pl.id) {
                            Service.getInstance().chat(pl, "Ôi xinh quá");
                        }
                    }
                }
                this.lastTimeGaiXink = System.currentTimeMillis();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
