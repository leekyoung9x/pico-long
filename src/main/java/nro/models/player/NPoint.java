...
public class NPoint {
    ...
    public boolean isNTKSL;
    public boolean isX3TnsmHTTV;
    public boolean isXTNSM;
    public boolean isStone;
    public boolean isGaiXink;
    public byte limitPower;
    public long power;
    public long tiemNang;
    public int isXTNSMVal;
    ...
    public int tlGiamst;
    public boolean isOptionCell;

    public boolean isOptionx2;
    ...
    public void setPoint(ItemOption io) {
        ...
        switch (io.optionTemplate.id) {
            ...
            case 160:
                ...
                break;
            case 246:
                if (this.player.pet != null && player.pet.typePet == ConstPet.BILL_CON) {
                    this.tlTNSMPet += io.param;
                }
                break;
            ...
            case 231:
                ...
                break;
            case 234:
                this.isX2ItemReward = true;
                break;
            case 239:
                this.isOptionCell = true;
                break;
            case 240:
                this.isOptionx2 = true;
                break;
            case 241:
                this.isStone = true;
                break;
            case 247:
                this.isX3TnsmHTTV = true;
                break;
            case 175:
                this.isXTNSM = true;
                this.isXTNSMVal = io.param;
                break;
            case 183:
                this.isGaiXink = true;
                break;
            ...
            case 43:
                ...
                break;
            case 44:
                if (this.player.pet != null && player.pet.typePet == ConstPet.WHIS) {
                    this.tlDame.add(io.param / chiSoChiaSD);
                    this.tlHp.add(io.param / chiSoChiaHP);
                    this.tlMp.add(io.param / chiSoChiaKI);
                }
                break;
            case 41: // Tăng #% sức đánh chí mạng nếu sở hữu đệ Whis 150 tỷ sức mạnh
                if (this.player.pet != null && player.pet.typePet == ConstPet.WHIS && player.pet.nPoint.power >= 150_000_000_000L) {
                    this.tlDameCrit.add(io.param);
                }
                break;
            case 45: // Tăng #% Chỉ số nếu sở hữu đệ Whis 120 tỷ sức mạnh
                if (this.player.pet != null && player.pet.typePet == ConstPet.WHIS && player.pet.nPoint.power >= 120_000_000_000L) {
                    this.tlDame.add(io.param / chiSoChiaSD);
                    this.tlHp.add(io.param / chiSoChiaHP);
                    this.tlMp.add(io.param / chiSoChiaKI);
                }
                break;
            case 253: //Sức đánh+#%
                if (player.setClothes.setSucDanh == 5) {
                    this.tlDame.add(io.param / chiSoChiaSD);
                }
                break;
            case 169: // đẹp trai + #% sđ, hp, ki
                this.tlDame.add(io.param / chiSoChiaSD);
                this.tlHp.add(io.param / chiSoChiaSD);
                this.tlMp.add(io.param / chiSoChiaSD);
                break;
            case 187: // kháng nóng
                this.isKhongNong = true;
                break;
            case 200: // Tăng #% Chỉ số nếu sở hữu đệ Zeno
                if (this.player.pet != null && player.pet.typePet == ConstPet.ZENO) {
                    this.tlDame.add(io.param / chiSoChiaSD);
                    this.tlHp.add(io.param / chiSoChiaHP);
                    this.tlMp.add(io.param / chiSoChiaKI);
                }
                    ...
                    for (ItemOption io : item.itemOptions) {
                        switch (io.optionTemplate.id) {
                            ...
                            case 14:// chí mạng
                                this.critAdd += io.param;
                                break;
                            case 245:// Sức mạnh rồng thiêng tăng 10% SĐ,HP,KI
                                this.tlDame.add(io.param);
                                this.tlHp.add(io.param);
                                this.tlMp.add(io.param);
                                break;
                            case 169:// Sao hạng A tăng 10% SĐ,HP,KI
                                this.tlDame.add(io.param);
                                this.tlHp.add(io.param);
                                this.tlMp.add(io.param);
                                break;
        ...
        if (!this.player.isPet && !this.player.isBoss && !this.player.isMiniPet) {
            try {
                ...
                if (gtl.isNotNullItem()) {
                    ...
                    // ekko giáp luyện tập chỉ tác dụng sức đánh
                    this.tlSubSD += ItemService.gI().getPercentTrainArmor(gtl);
//                    this.tlSubHp += ItemService.gI().getPercentTrainArmor(gtl);
//                    this.tlSubMp += ItemService.gI().getPercentTrainArmor(gtl);
                } else {
                    if (this.wornTrainArmor) {
                        ...
                        for (ItemOption io : this.player.inventory.trainArmor.itemOptions) {
                            if (io.optionTemplate.id == 9 && io.param > 0) {
                                ...
                                //this.tlHp.add(ItemService.gI().getPercentTrainArmor(this.player.inventory.trainArmor));
                                //this.tlMp.add(ItemService.gI().getPercentTrainArmor(this.player.inventory.trainArmor));
                                break;
                            }
                        }
                    }
...
//        if (hp >= 2000000000 || hpMax >= 2000000000 && !this.player.isBoss) {
...
//        }
        ...
        if (this.player.setClothes.setKHA == 5) {
        ...
        if (this.player.nPoint.isOptionCell) {
            if (this.player.pet != null) {
                if (this.player.pet.typePet == ConstPet.WHIS) {
                    this.hpMax += calPercent(this.hpMax, 10);
                }
            }
        }

        if (this.player.nPoint.isOptionx2) {
            this.hpMax += calPercent(this.hpMax, 50);
        }

        if (this.player.isPet) {
            int level = ((Pet) this.player).getLever();
            int percent = level * 3;
            if (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA
                ...
                switch (((Pet) this.player).typePet) {
                    ...
                    case ConstPet.WHIS:
                        ...
                        int perAddWhis = 40;
                        if(level >= 7) {
                            // tăng 60% chỉ số khi hợp thể
                            perAddWhis = 60;
                        }
                        this.hpMax += calPercent(this.hpMax, perAddWhis);
                        break;
                    case ConstPet.CELL:
                        this.hpMax += calPercent(this.hpMax, 20 + percent);
                        break;
                    ...
                    case ConstPet.ZENO:
                        int perAddZeno = 0;
                        if(level >= 7) {
                            // tăng 100% chỉ số khi hợp thể
                            perAddZeno = 100;
                        }
                        this.hpMax += calPercent(this.hpMax, perAddZeno);
                        break;
                    case ConstPet.BLACK_GOKU:
                        // tăng 120% chỉ số khi hợp thể
                        int perAddGoku = 120;
                        this.hpMax += calPercent(this.hpMax, perAddGoku);
                        break;
//                    case ConstPet.BLACK_GOKU: {
//                        int perAddGoku = 120;
//                        this.hpMax += (int) calPercent(this.hpMax, perAddGoku);
//                        break;
//                    }
        ...
        this.hpMax -= calPercent(this.hpMax, tlSubHp);
...
//        }
        ...
        if (this.player.itemTime != null && this.player.itemTime.isUsebinhtangluc) {
            this.hpMax += calPercent(this.hpMax, 10);
        }
...
//        }
        ...
        // ekko giảm 1 nửa chỉ số khi vào map thung lũng đá
        if (this.player.zone != null && MapService.gI().isMapThungLungDa(this.player.zone.map.mapId) && !this.isKhongNong ) {
            this.hpMax /= 2;
        }
        // ekko map doanh trại / 5
        if (this.player.zone != null && this.player.zone.map != null && MapService.gI().isMapDoanhTrai(this.player.zone.map.mapId)) {
            this.hpMax /= 5;
        }
        if (!player.isBoss) {
        ...
        if (this.player.getSession() != null && this.player.isPl()) {
            if (this.player.getSession().actived >= 1) {
                hpMax += calPercent(hpMax, 10);
            }
        }
        ...
        // tăng 10% hp
        if (this.player.itemTime != null && this.player.itemTime.rateNoelDrHPKI) {
            this.hpMax += calPercent(this.hpMax, 10);
        }
        // tăng 10% hp
        if (this.player.itemTime != null && this.player.itemTime.rateNaoCho) {
            this.hpMax += calPercent(this.hpMax, 10);
        }
        // tăng 10% HP
        if (this.player.itemTime != null && this.player.itemTime.rateNoelDrSucDanhHPKI) {
            this.hpMax += calPercent(this.hpMax, 10);
        }
        // ekko năng lượng bill xd tăng 15% HP
        if (this.player.isPl() && this.player.itemTime.isNangLuongBill && this.player.gender == 2) {
            this.hpMax += calPercent(this.hpMax, 15);
        }
        // ekko đậu thần cấp 11 tăng 10% HP
        if (this.player.isPl() && this.player.itemTime.isDauThanCap11) {
            this.hpMax += calPercent(this.hpMax, 10);
        }
    ...
    private int GetValueOptionOfSpecialPlayer(short id) {
        for (PlayerSpecialAura aura : Manager.PLAYER_SPECIAL_AURA) {
            if (this.player.id == aura.player_id && aura.status == 1) {
                for (ItemOption io : aura.options) {
    ...
    private void setMpMax() {
        ...
        this.mpMax -= calPercent(this.mpMax, tlSubMp);
        //ngọc rồng đen 3 sao
            ...
            if (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA
                ...
                switch (((Pet) this.player).typePet) {
                    ...
                    case ConstPet.WHIS:
                        ...
                        this.mpMax += calPercent(this.mpMax, perAddWhis);
                        break;
                    case ConstPet.CELL:
                        this.mpMax += calPercent(this.mpMax, 20 + percent);
                        break;
                    case ConstPet.MABU_NHI:
                    ...
                    case ConstPet.BILL_CON:
                        ...
                        break;
                    case ConstPet.ZENO:
                        // tăng 0% chỉ số khi hợp thể
                        int perAddZeno = 0;
                        if(level >= 7) {
                            // tăng 100% chỉ số khi hợp thể
                            perAddZeno = 100;
                        }
                        this.mpMax += calPercent(this.mpMax, perAddZeno);
                        break;
                    case ConstPet.BLACK_GOKU:
                        // tăng 120% chỉ số khi hợp thể
                        int perAddGoku = 120;
                        this.mpMax += calPercent(this.mpMax, perAddGoku);
                        break;
//                    case ConstPet.BLACK_GOKU: {
//                        perAddGoku = 120;
//                        this.mpMax += (int) calPercent(this.mpMax, perAddGoku);
//                        break;
//                    }
                }
            }
...
//        }
        ...
        // ekko năng lượng bill nm tăng 15% KI
        if (this.player.isPl() && this.player.itemTime.isNangLuongBill && this.player.gender == 1) {
            this.mpMax += calPercent(this.mpMax, 15);
        }
        // ekko đậu thần cấp 11 tăng 10% KI
        if (this.player.isPl() && this.player.itemTime.isDauThanCap11) {
            this.mpMax += calPercent(this.mpMax, 10);
        }
        // Map Mabu 6h Sửa lại vào Map chỉ số SĐ HP KI Bị chia 5 Lần
    ...
    private void setDame() {
        ...
        this.dame += (int) calPercent(this.dame, GetValueOptionOfSpecialPlayer(ConstOption.SUC_DANH_CONG_PHAN_TRAM));
            ...
            if (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA
                ...
                switch (((Pet) this.player).typePet) {
                    ...
                    case ConstPet.WHIS:
                        ...
                        this.dame += calPercent(this.dame, perAddWhis);
                        break;
                    case ConstPet.CELL:
                        this.dame += calPercent(this.dame, 20 + percent);
                        break;
                    case ConstPet.MABU_NHI:
                    ...
                    case ConstPet.ZENO:
                        ...
                        this.dame += calPercent(this.dame, perAddZeno);
                        break;
                    case ConstPet.BLACK_GOKU:
                        // tăng 120% chỉ số khi hợp thể
                        int perAddGoku = 120;
                        this.dame += calPercent(this.dame, perAddGoku);
                        break;
//                    case ConstPet.BLACK_GOKU: {
//                        perAddGoku = 120;
//                        this.dame += (int) calPercent(this.dame, perAddGoku);
//                        break;
//                    }
                }
            }
...
//        }
        ...
        // ekko map thung lũng đá giảm 1 nửa sức đánh
        if (this.player.zone != null && MapService.gI().isMapThungLungDa(this.player.zone.map.mapId) && !this.isKhongNong) {
            this.dame /= 2;
        }
        // ekko map doanh trại / 5
        ...
        // đậu thần cấp 11 tăng 10% sức đánh
        if (this.player.itemTime != null && this.player.itemTime.isDauThanCap11) {
            this.dame += calPercent(this.dame, 10);
        }
        // Map Mabu 6h Sửa lại vào Map chỉ số SĐ HP KI Bị chia 5 Lần
        if (this.player.zone != null && MapService.gI().isMapMabuWar14H(this.player.zone.map.mapId)) {
    ...
    private boolean isGenderSubDame() {
        return this.player.gender == 0 || this.player.gender == 2;
    }
    ...
    private void setDef() {
        ...
        // ekko map thung lũng đá giảm 1 nửa giáp
        if (this.player.zone != null && MapService.gI().isMapThungLungDa(this.player.zone.map.mapId) && !this.isKhongNong) {
            this.def /= 2;
        }
    ...
    private void setCrit() {
        ...
        // ekko map thung lũng đá giảm 1 nửa chí mạng
        if (this.player.zone != null && MapService.gI().isMapThungLungDa(this.player.zone.map.mapId) && !this.isKhongNong) {
            this.crit /= 2;
        }
    ...
    private void setCritDame() {
        ...
        // ekko năng lượng bill tăng 10% sức đánh chí mạng
        if (this.player.itemTime != null && this.player.itemTime.isNangLuongBill) {
    ...
    private void resetPoint() {
        ...
        this.tlGiamst = 0;
        this.tlTNSMPet = 0;
        this.isX2ItemReward = false;
        this.isOptionCell = false;
        this.isOptionx2 = false;
        this.isStone = false;
        this.isNTKSL = false;

        this.isX3TnsmHTTV = false;
        this.isXTNSM = false;
        this.isGaiXink = false;
        this.isXTNSMVal = 0;
...
//                    // ekko
//                    // đủ set goku tăng 100% sát thương KameJoKo
//                    percentXDame = 100;
//                }
...
//                tiemNang += tn * 2;
            ...
            if (mapid == 212) {
                tiemNang += (tiemNang * 300);
            }
            if (this.player.isPet && mapid == 212) {
                tiemNang -= (tiemNang * 80 / 100);
            }

//            if (this.player.isPet && mapid == 212) {
//                if (player.pet != null && player.pet.nPoint != null && player.pet.nPoint.power < 100_000_000_000L) {
...
//            }
            ...
            if (this.player.isPet) {
                ...
                if (((Pet) this.player).typePet == ConstPet.MABU_NHI) {
                    tiemNang /= 100;
                }
                if (((Pet) this.player).typePet == ConstPet.BILL_CON) {
                    // bill nhí = 1 / 5 so với đệ thường
                    tiemNang /= 5;
                }
                if (((Pet) this.player).typePet == ConstPet.WHIS) {
                    // Whis = 1 / 50 so với đệ thường
                    tiemNang /= 50;

                    Player master = ((Pet) player).master;

                    // x5 TNSM cho đệ whis
                    if (master.itemTime != null && master.itemTime.rateBoneDrTNSM) {
                        tiemNang *= 5;
                    }
                    //System.out.println(player.nPoint.tiemNang);
                }
                if (((Pet) this.player).typePet == ConstPet.ZENO) {
                    // Zeno = 1 / 100 so với đệ thường
                    tiemNang /= 100;
                }
                if (((Pet) this.player).typePet == ConstPet.BLACK_GOKU) {
                    // Black Goku = 1 / 100 so với đệ thường
                    tiemNang /= 100;
                }
...
//            for (Item itembody : player.inventory.itemsBody) {
//                if (Objects.isNull(itembody)) {
//                    continue;
//                }
//                int param = itembody.getParams(optionId);
//                listParams.add(param);
//            }
//            if (listParams.isEmpty()) {
//                return 0;
//            }
//            return listParams.stream().sorted().findFirst().get();
//        }
//        return 0;
//    }
...
//    }
    ...
    public void addNangDong(int nangdong) {
        this.nangdong += nangdong;
        int limit = getNangDongLimit();
        if (this.nangdong > limit) {
            this.nangdong = limit;
        }
        if (this.nangdong < 0) {
            this.nangdong = 0;
    ...
    public int getNangDongLimit() {
        return nangdong + 2000000000;
    }
}
