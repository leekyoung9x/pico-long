...
public class BillEgg {
    ...
    private static final long DEFAULT_TIME_DONE = 604800000;
...
//        System.out.println("nro.models.npc.specialnpc.BillEgg.createBillEgg()");
    ...
    public void openEggBill(int gender) {
        if (this.player.pet != null) {
            try {
                destroyEggBill();
                Thread.sleep(4000);
                PetService.gI().createBILLCON(this.player, gender);
                ChangeMapService.gI().changeMapInYard(this.player, 154, -1, Util.nextInt(300, 500));
                player.billEgg = null;
            } catch (Exception e) {
            }
        } else {
