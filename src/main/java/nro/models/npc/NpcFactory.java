...
import nro.models.item.RewardItem;
        ...
        try {
            switch (tempId) {
                case ConstNpc.CAY_NEU: {
                    npc = new CayNeu(mapId, status, cx, cy, tempId, avartar);
                    break;
                }
                case ConstNpc.MR_POPO:{
                     npc = new Mr_Popo(mapId, status, cx, cy, tempId, avartar);
                     break;
                }
                ...
                case ConstNpc.DUONG_TANG: {
                    npc = new duongtank(mapId, status, cx, cy, tempId, avartar);
                    break;
                }

                case ConstNpc.NGO_KHONG: {
                    npc = new ngok(mapId, status, cx, cy, tempId, avartar);
                    break;
                }
                case ConstNpc.JACO: {
                ...
                case ConstNpc.ONGLAO: {
                    npc = new Onglao(mapId, status, cx, cy, tempId, avartar);
                    break;
                }
                case ConstNpc.DENDE: {
                ...
                case ConstNpc.QUA_TRUNG2: {
                    npc = new egg2(mapId, status, cx, cy, tempId, avartar);
                    break;
                }
                ...
                case ConstNpc.DOC_NHAN: {
                    npc = new DocNhan(mapId, status, cx, cy, tempId, avartar);
                    break;
                }
                case ConstNpc.GIUMA_DAU_BO: {
                    npc = new GiumaDauBo(mapId, status, cx, cy, tempId, avartar);
                    break;
                }
                default:
            ...
            public void confirmMenu(Player player, int select) {
                switch (player.iDMark.getIndexMenu()) {
                    ...
                    case ConstNpc.MENU_USE_ITEM_BLACK_GOKU:
                        ...
                        break;
                    case ConstNpc.MENU_OPTION_USE_ITEM1961:
                        switch (select) {
                            case 0:
                                IntrinsicService.gI().set_TD(player);
                                break;
                            case 1:
                                IntrinsicService.gI().set_NM(player);
                                break;
                            case 2:
                                IntrinsicService.gI().set_XD(player);
                                break;
                            default:
                                break;
                    ...
                    case ConstNpc.MENU_TD:
                        switch (select) {
                            case 0: // set songoku
                                RewardItem.gI().setSongoku(player);
                                break;
                            case 1:// set kaioken
                                RewardItem.gI().setKaioKen(player);
                                break;
                            case 2:// set thenxin hang
                                RewardItem.gI().setThenXinHang(player);
                                break;
                        }
                        break;
                    case ConstNpc.MENU_NM:
                        switch (select) {
                            case 0:
                                RewardItem.gI().setLienHoan(player);
                                break;
                            case 1:
                                RewardItem.gI().setPicolo(player);
                                break;
                            case 2:
                                RewardItem.gI().setPikkoroDaimao(player);
                                break;
                        }
                        break;
                    ...
                    case ConstNpc.MENU_XD:
                        switch (select) {
                            case 0:
                                RewardItem.gI().setKakarot(player);
                                break;
                            case 1:
                                RewardItem.gI().setCadic(player);
                                break;
                            case 2:
                                RewardItem.gI().setNappa(player);
                                break;
                        }
                        break;
                    ...
                    case ConstNpc.MENU_OPTION_USE_ITEM2167:
                        switch (select) {
                            case 0:
                                IntrinsicService.gI().setTDHD(player);
                                break;
                            case 1:
                                IntrinsicService.gI().setNMHD(player);
                                break;
                            case 2:
                        ...
                        break;
                    case ConstNpc.MENU_OPTION_USE_ITEM_HOP_BTC3:
                        switch (select) {
                            case 0:
                                try {
                                    ItemService.gI().bongTaiCap3MaxChiSo(player, ConstOptionSelect.SUC_DANH);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                break;
                            case 1:
                                try {
                                    ItemService.gI().bongTaiCap3MaxChiSo(player, ConstOptionSelect.HP);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                break;
                            case 2:
                                try {
                                    ItemService.gI().bongTaiCap3MaxChiSo(player, ConstOptionSelect.KI);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                break;
                        ...
                        break;
                    case ConstNpc.MENU_OPTION_USE_ITEM:
                        ItemService.gI().setThanLinh(player, select);
                        break;
                    case ConstNpc.menutd:
                        switch (select) {
                            ...
                            case 2:// set Krillin
                                   try {
                                ItemService.gI().setKrillin(player);
                            } catch (Exception e) {
                            }
                            break;
                        }
                    ...
                    case ConstNpc.menunm:
                        switch (select) {
                            case 0:
                                try {
                                ItemService.gI().setDende(player);
                            } catch (Exception e) {
                            }
                            break;
                            case 1:
                            ...
                            case 2:
                                try {
                                ItemService.gI().setDaimao(player);
                            } catch (Exception e) {
                            }
                            break;
                        }
                    ...
                    case ConstNpc.MENU_TD_HD:
                        switch (select) {
                            case 0: {// set songoku
                                try {
                                    ItemService.gI().setSongokuHD(player);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            }
                            ...
                            case 1:// set Gohan
                                try {
                                    ItemService.gI().setGohanHD(player);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                break;
                            case 2:// set Krillin
                                try {
                                    ItemService.gI().setKrillinHD(player);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                break;
                    ...
                    case ConstNpc.MENU_NM_HD:
                        switch (select) {
                            case 0:
                                try {
                                    ItemService.gI().setDendeHD(player);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                break;
                            case 1:
                                try {
                                    ItemService.gI().setPicoloHD(player);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                break;
                            case 2:
                                try {
                                    ItemService.gI().setDaimaoHD(player);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                break;
                    ...
                    case ConstNpc.MENU_XD_HD:
                        switch (select) {
                            case 0:
                                try {
                                    ItemService.gI().setKakarotHD(player);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                break;
                            case 1:
                                try {
                                    ItemService.gI().setCadicHD(player);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                break;
                            case 2:
                                try {
                                    ItemService.gI().setNappaHD(player);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                break;
                    ...
                    case ConstNpc.MENU_MABU_WAR:
                        if (select == 0) {
                            ...
                            if (Util.isTrue(1, 30)) {
                                ...
                                player.inventory.addRuby(1);
                                // ekko ghi log add ruby
                                Manager.addPlayerRubyHistory(player.id, oldRuby, player.inventory.ruby, "NpcFactory-createNpcConMeo");
                    ...
                    case ConstNpc.SUMMON_ICE_SHENRON:
                        ...
                        break;
                    case ConstNpc.SUMMON_DRAGON_NOEL:
                        if (select == 0) {
                            SummonDragon.gI().summonDragonNoel(player);
                        }
                        break;
...
//                                Service.getInstance().chat(player, "Cập nhật thông báo thành công");
                    ...
                    case ConstNpc.DE_TU_ZENO:
                        switch (select) {
                            case 0 -> {
                                Item zn = InventoryService.gI().findItem(player.inventory.itemsBag, ConstItem.DE_TU_ZENO);
                                if (zn != null) {
                                    if (player != null && player.pet != null && player.pet.typePet == ConstPet.WHIS && player.pet.nPoint.power >= 100_000_000_000L) {
                                        // đổi sang pet Zeno
                                        PetService.gI().changeZenoPetByLevel(player, player.gender, player.pet.lever);
                                        Service.getInstance().sendThongBao(player, "Chúc mừng ngươi có đệ tử mới");
                                        InventoryService.gI().subQuantityItemsBag(player, zn, 1);
                                        InventoryService.gI().sendItemBags(player);
                                    } else {
                                        Service.getInstance().sendThongBao(player, "Ngươi phải sở hữu đệ Whis 100 tỷ sức mạnh trở lên");
                                    }
                                }
                            }
                        }
                        break;
                    case ConstNpc.DE_TU_WHIS:
                        switch (select) {
                            case 0 -> {
                                Item whis = InventoryService.gI().findItem(player.inventory.itemsBag, ConstItem.DE_TU_WHIS);
                                if (whis != null) {
                                    if (player != null && player.pet != null && player.pet.typePet == ConstPet.BILL_CON && player.pet.lever >= 7) {
                                        // đổi sang pet whis
                                        PetService.gI().changeWhisPetByLevel(player, player.gender, player.pet.lever);
                                        Service.getInstance().sendThongBao(player, "Chúc mừng ngươi có đệ tử mới");
                                        InventoryService.gI().subQuantityItemsBag(player, whis, 1);
                                        InventoryService.gI().sendItemBags(player);
                                    } else {
                                        Service.getInstance().sendThongBao(player, "Ngươi phải sở hữu đệ Bill Level 7 trở lên");
                                    }
                                }
                            }
                        }
                        break;
                    case ConstNpc.DE_TU_BLACK_GOKU:
                        switch (select) {
                            case 0 -> {
                                Item goku = InventoryService.gI().findItem(player.inventory.itemsBag, ConstItem.DE_TU_BLACK_GOKU);
                                if (goku != null) {
                                    if (player != null && player.pet != null && (player.pet.typePet == ConstPet.WHIS || player.pet.typePet == ConstPet.ZENO) && player.pet.nPoint.power >= 110_000_000_000L) {
                                        // đổi sang pet goku
                                        if (Util.isTrue(1, 100)) {
                                            PetService.gI().changeGokuPetByLevel(player, player.gender, player.pet.lever);
                                            Service.getInstance().sendThongBao(player, "Chúc mừng ngươi có đệ tử mới");
                                        } else {
                                            Service.getInstance().sendThongBao(player, "Đen cho ngươi quá, chúng ta không thuộc về nhau rồi");
                                        }
                                        InventoryService.gI().subQuantityItemsBag(player, goku, 1);
                                        InventoryService.gI().sendItemBags(player);
                                    } else {
                                        Service.getInstance().sendThongBao(player, "Ngươi phải sở hữu đệ Whis hoặc đệ Zeno đạt 110 tỷ sức mạnh trở lên");
                                    }
                                }
                            }
                        }
                        ...
                        if (!player.event.isReceivedLuckyMoney()) {
                            ...
                            if (day >= 22 && day <= 24) {
                                Item goldBar = ItemService.gI().createNewItem((short) ConstItem.THOI_VANG,
                                        Util.nextInt(1, 3));
                                int rubyAdd = Util.nextInt(10, 30);
                                int oldRuby = player.inventory.ruby;
                                player.inventory.addRuby(rubyAdd);
                                // ekko ghi log add ruby
                                Manager.addPlayerRubyHistory(player.id, oldRuby, player.inventory.ruby, "NpcFactory-openMenuSuKien");
