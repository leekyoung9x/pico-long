                    ...
                    if (plSameClans.size() >= 2) {
                        if (!player.isAdmin() && player.clanMember
                            ...
                            createOtherMenu(player, ConstNpc.MENU_KHONG_CHO_VAO_DT,
                                    "Bang hội chỉ cho phép những người ở trong bang trên 2 ngày. Hẹn ngươi quay lại vào lúc khác",
                                    "OK", "Hướng\ndẫn\nthêm");
                        ...
                        createOtherMenu(player, ConstNpc.MENU_KHONG_CHO_VAO_DT,
                                "Ngươi phải có ít nhất 2 đồng đội cùng bang đứng gần mới có thể\nvào\n"
                                        + "tuy nhiên ta khuyên ngươi nên đi cùng với 5 - 6 người để khỏi chết.\n"
                                        + "Hahaha.",
                                "OK", "Hướng\ndẫn\nthêm");
                    }
                }
            ...
            if (this.mapId == 27) {
                switch (player.iDMark.getIndexMenu()) {
                    case ConstNpc.MENU_KHONG_CHO_VAO_DT:
                        ...
                        if (select == 2) {
                            MenuDialog menu = new MenuDialog("Ngươi muốn hối lộ ta để đi thêm lần nữa à", new String[]{"Đúng vậy", "Đéo"}, new MenuRunable() {
                                @Override
                                public void run() {
                                    switch (getIndexSelected()) {
                                        case 0:
                                            ...
                                            InventoryService.gI().subQuantityItemsBag(player, item, 50_000);
                                            InventoryService.gI().sendItemBags(player);
                                            player.clan.haveGoneDoanhTrai = false;
                                            createOtherMenu(player, ConstNpc.MENU_CHO_VAO_DT,
                                                    "Haha ngươi cũng hiểu chuyện ấy\n",
                                                    "Vào\n(miễn phí)", "Không", "Hướng\ndẫn\nthêm");
                                            break;
                                        case 1:
                                            break;
