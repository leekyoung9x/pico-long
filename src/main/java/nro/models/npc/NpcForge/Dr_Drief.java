...
public class Dr_Drief extends Npc {
    ...
    public void openBaseMenu(Player pl) {
        if (canOpenNpc(pl)) {
            ...
            } else if (this.mapId == ConstMap.LANH_DIA_BANG_HOI) {
                Clan clan = pl.clan;
...
//                            "Từ chối");
                    ...
                    Item i = InventoryService.gI().findItemBagByTemp(pl, ConstItem.CAPSULE_BANG_HOI);
                    ...
                    this.createOtherMenu(pl, ConstNpc.BASE_MENU,
                            "Cần 99 capsule bang [đang có " + quantity
                                    + " capsule bang] để nâng cấp bang hội lên cấp "
                                    + (clan.level++) + "\n"
                                    + "+1 tối đa số lượng thành viên",
                            "Về\nĐảo Kame", "Từ chối");
...
//                    this.createOtherMenu(pl, ConstNpc.BASE_MENU,
//                            "Cần 99 capsule bang [đang có " + clan.clanPoint
//                                    + " capsule bang] để nâng cấp bang hội lên cấp "
//                                    + (clan.level++) + "\n"
//                                    + "+1 tối đa số lượng thành viên",
//                            "Về\nĐảo Kame", "Từ chối");
...
//                            "Về\nĐảoKame", "Đồng ý", "Từ chối");
    ...
    public void confirmMenu(Player player, int select) {
        if (canOpenNpc(player)) {
            ...
            } else if (mapId == ConstMap.LANH_DIA_BANG_HOI) {
                if (select == 0) {
