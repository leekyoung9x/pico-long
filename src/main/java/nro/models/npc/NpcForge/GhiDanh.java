        ...
        if (canOpenNpc(player)) {
            switch (this.mapId) {
                case ConstMap.DAI_HOI_VO_THUAT:
                    this.createOtherMenu(player, ConstNpc.BASE_MENU,//IGNORE_MENU
                            ...
                            switch (select) {
                                case 0:
                                    if (player.getSession().actived == 0) {
                                        Service.getInstance().sendThongBao(player, "Hãy mở VIP để tham gia giải đấu võ thuật");
                                        return;
                                    }
                                    if (InventoryService.gI().finditemWoodChest(player)) {
                                        if (player.inventory.getGem()>= goldchallenge) {
                                            MartialCongressService.gI().startChallenge(player);
                                            player.inventory.subGem(goldchallenge);
                                            PlayerService.gI().sendInfoHpMpMoney(player);
                                            player.goldChallenge += 10;
                                        } else {
                                            Service.getInstance().sendThongBao(player, "Không đủ vàng, còn thiếu "
                                                    + Util.numberToMoney(goldchallenge - player.inventory.gem) + " ngọc xanh");
                                        }
                                    } else {
