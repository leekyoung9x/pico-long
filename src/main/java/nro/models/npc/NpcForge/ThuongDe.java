                ...
                if (player.iDMark.isBaseMenu()) {
                    switch (select) {
                        ...
                        case 1:
                            if (player.nPoint.power < 80_000_000_000L) {
                                this.createOtherMenu(player, ConstNpc.IGNORE_MENU, "em chưa đủ 80 tỷ ? em cúc ra cho toi", "dạ");
                            }
                            else if(player.getSession().actived == 0) {
                                this.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Con chưa mở VIP 1 ? Con cook đi cho ta, mở VIP 1 xong hãng gặp ta nha!", "Dạ");
                            }
                            else {
                                this.createOtherMenu(player, ConstNpc.MENU_CHOOSE_LUCKY_ROUND,
                                        "Con muốn làm gì nào?",
                                        "Quay bằng\n" + ConstItemName.MONEY_UNIT,
                                        "Quay bằng\nhồng ngọc",
                                        "Rương phụ\n("
                    ...
                    switch (select) {
                        case 0:
                            ...
                            break;
                        case 1:
                            LuckyRoundService.gI().openCrackBallUI(player, LuckyRoundService.USING_GEM);
                            break;
                        case 2:
