        ...
        try {
            ...
            try {
                if (rs.next()) {
                    ...
                    try {
                        if (clanId != -1) {
                            Clan clan = ClanService.gI().getClanById(player.id, clanId);
                            if (clan != null) {
                        ...
                        System.out.println("Lỗi load clan idđ");
                    }
                    ...
                    player.lvpet = rs.getShort("lvpet");
                    ...
                    try {
                        ...
                        if (MapService.gI().isMapHTTV(mapId) || MapService.gI().isMapDoanhTrai(mapId) || MapService.gI().isMapBlackBallWar(mapId)
                    ...
                    player.nPoint.nangdong = Integer.parseInt(dataArray.get(0).toString());
                    ...
                    player.nPoint.defg = Long.parseLong(dataArray.get(7).toString());
                    ...
                    for (int i = 0; i < dataArray.size(); i++) {
                        ...
                        boolean spl = false;
                        ItemOption splcs = new ItemOption(107, 0);
                        ...
                        if (tempId != -1) {
                            ...
                            for (int j = 0; j < options.size(); j++) {
                                ...
                                int optionValue = Integer.parseInt(String.valueOf(opt.get(1)));
                                ...
                                switch (optionId) {
                                    ...
                                    case 107:
                                        if (!spl) {
                                            spl = true;
                                            splcs = new ItemOption(optionId, optionValue);
                                        }
                                        break;
                                    default:
                    ...
                    // xóa 3 ô cuối đi vì đang tạm đóng danh hiệu, Aether Stone, Chân mệnh
                    // mở danh hiệu
//                    if(player.inventory.itemsBody.size() > 12) {
//                        // Lấy 12 phần tử đầu tiên
//                        List<Item> subList = player.inventory.itemsBody.subList(0, 12);
//                        player.inventory.itemsBody = new ArrayList<>(subList);
//                    }
                    ...
                    // nếu thiếu item thì add số 0 vào để khi thêm item mới vào thì không bị lỗi
                    int totalItem = 14;
                    if (dataArray.size() < totalItem) {
                        int arrSize = dataArray.size();
                        for (int i = arrSize; i < totalItem; i++) {
                            dataArray.add(0);
                        }
                    }
                    ...
                    int timeMa = 0;
                    int item212x2 = 0;
                    int rateBoneDrTNSM = 0;
                    int rateBoneDrSucDanhHPKI = 0;
                    int rateBoneDrHPKI = 0;
                    int rateDameMaXuong = 0;
                    int rateNaoCho = 0;
                    int nangLuongBill = 0;
                    int dauThanCap11 = 0;
                    if (dataArray.size() >= 7) {
                        ...
                        rateBoneDrTNSM = Integer.parseInt(String.valueOf(dataArray.get(7)));
                        rateBoneDrSucDanhHPKI = Integer.parseInt(String.valueOf(dataArray.get(8)));
                        rateBoneDrHPKI = Integer.parseInt(String.valueOf(dataArray.get(9)));
                        rateDameMaXuong = Integer.parseInt(String.valueOf(dataArray.get(10)));
                        rateNaoCho = Integer.parseInt(String.valueOf(dataArray.get(11)));
                        nangLuongBill = Integer.parseInt(String.valueOf(dataArray.get(12)));
...
//                        item212x2 = Integer.parseInt(String.valueOf(dataArray.get(7)));
                    ...
                    player.itemTime.lastTimeDameDr = System.currentTimeMillis() - (ItemTime.ONE_HOUR - rateDame);
                    player.itemTime.lastTimeitem212x2 = System.currentTimeMillis()- (ItemTime.TIME_ITEM - item212x2);
                    ...
                    player.itemTime.lastTimeTNSMBoneDr = System.currentTimeMillis() - (ItemTime.THIRTY_MINUTES - rateBoneDrTNSM);
                    player.itemTime.lastTimeSucDanhHPKINoelDr = System.currentTimeMillis() - (ItemTime.TEN_MINUTES - rateBoneDrSucDanhHPKI);
                    player.itemTime.lastTimeHPKINoelDr = System.currentTimeMillis() - (ItemTime.THIRTY_MINUTES - rateBoneDrHPKI);
                    player.itemTime.lastTimeDameMaXuong = System.currentTimeMillis() - (ItemTime.DAME_MAXUONG - rateDameMaXuong);
                    ...
                    player.itemTime.item212x2 = item212x2 != 0;
                    player.itemTime.rateBoneDrTNSM = rateBoneDrTNSM != 0;
                    player.itemTime.rateNoelDrSucDanhHPKI = rateBoneDrSucDanhHPKI != 0;
                    player.itemTime.rateNoelDrHPKI = rateBoneDrHPKI != 0;
                    player.itemTime.rateDameMaXuong = rateDameMaXuong != 0;
                    player.itemTime.rateNaoCho = rateNaoCho != 0;
                    player.itemTime.isNangLuongBill = nangLuongBill != 0;
                    player.itemTime.isDauThanCap11 = dauThanCap11 != 0;
                    dataArray.clear();
                    ...
                    try {
                        ...
                        // ekko thời gian hóa xương
                        dataArray.clear();
                        dataArray = (JSONArray) jv.parse(rs.getString("time_effect"));
                        long timeHoaXuong = Long.parseLong(String.valueOf(dataArray.get(0)));
                        player.effectSkin.lastTimeUseHoaXuong = timeHoaXuong;
                        dataArray.clear();
                    } catch (Exception e) {
                    }
                    ...
                    //data zama
                    dataArray = (JSONArray) jv.parse(rs.getString("zm"));
                    player.diemboss = Integer.parseInt(dataArray.get(0).toString());
                    int timebinhtangluc = Integer.parseInt(dataArray.get(1).toString());
                    player.itemTime.lastTimebinhtangluc = System.currentTimeMillis() - (ItemTime.TIME_ITEM - timebinhtangluc);
                    player.itemTime.isUsebinhtangluc = timebinhtangluc != 0;
                    dataArray.clear();
...
//                        System.out.println(player.name + " " + skillTemp.toJSONString());
...
//                    }
                    //data tổng njap
                    dataArray = (JSONArray) jv.parse(rs.getString("data_tong_nap"));
                    player.data_tong_nap = new ArrayList<>();
                    player.data_tong_nap.add(0);
                    player.data_tong_nap.add(0);
                    player.data_tong_nap.add(0);
                    try{
                        player.data_tong_nap.set(0, Integer.parseInt(dataArray.get(0).toString()));
                        player.data_tong_nap.set(1, Integer.parseInt(dataArray.get(1).toString()));
                        player.data_tong_nap.set(2, Integer.parseInt(dataArray.get(2).toString()));
                    }
                    catch (Exception e){
                    ...
                    // lấy thông tin nhận quà theo ngày của player
                    ps = connection.prepareStatement("select id, player_id, items from player_reward where player_id = ?");
                    ps.setLong(1, player.id);
                    ResultSet rsReward = ps.executeQuery();
                    if(rsReward.next()) {
                        PlayerReward reward = new PlayerReward();
                        reward.id = rsReward.getLong("id");
                        reward.player_id = rsReward.getLong("player_id");
                        reward.items = parseItemQuantities(rsReward.getString("items"));
                        player.playerReward = reward;
                    }
                    return player;
                }
            } finally {
