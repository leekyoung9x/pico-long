...
//            }
            ...
            if (MapService.gI().isMapBlackBallWar(player.zone.map.mapId)) {
                ...
                if ((currentTime - player.lastTimeHoiSinh) >= (secondsTarget * 1000)) {
                    player.lastTimeHoiSinh = currentTime;
                    Service.getInstance().hsChar(player, player.nPoint.hpMax, player.nPoint.mpMax);
                } else {
                    String timeLeft = TimeUtil.getTimeLeft(player.lastTimeHoiSinh, secondsTarget);
                    Service.getInstance().sendThongBaoOK(player, "Không thể hồi sinh, vui lòng đợi " + timeLeft);
                }
            }
            if (canHs) {
...
//                player.playerTask.achivements.get(ConstAchive.THANH_HOI_SINH).count++;
    ...
    public boolean savePlayer(Player player) {
        try {
            ...
            Connection conn = DBService.gI().getConnectionForLogout();
