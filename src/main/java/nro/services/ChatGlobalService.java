    ...
    public void chat(Player player, String text) {
        if (player.nPoint.power > 20_000_000_000L) {
            if (player.inventory.gem < 5) {
                Service.getInstance().sendThongBao(player, "Bạn chưa đủ ngọc xanh để chat thế giới");
                return;
            }
            long currentTime = System.currentTimeMillis();
            // chat thế giới giới hạn 30s được chat 1 lần
            if (Util.canDoWithTime(player.lastTimeChatTheGioi, 30_000)) {
                ...
                Service.getInstance().sendThongBao(player, "Vui lòng đợi " + ((player.lastTimeChatTheGioi + 30_000 - System.currentTimeMillis()) / 1000) + " giây để chat thế giới !");
            }
        } else {
