    ...
    public List<ItemMap> getRewardItems(Player player, Mob mob, int x, int yEnd) {
        ...
        if (mapid == 208) {
            if (Util.isTrue(1, 100)) {
                ItemMap itemMap = new ItemMap(mob.zone, 1950, 1, x, yEnd, player.id, player);
                list.add(itemMap);
            }
        }

        if (Util.isTrue(1, 10)) {
            ItemMap itemMap = new ItemMap(mob.zone, Util.getOne(77, 861), 10, x, yEnd, player.id, player);
            list.add(itemMap);
        }

        if (MapService.gI().isMapTuongLai(mapid)) {
            if (Util.isTrue(5, 200)) {
                ItemMap itemMap = new ItemMap(mob.zone, 1993, 1, x, yEnd, player.id, player);
                list.add(itemMap);
            }
            if (player.itemTime.isUseMayDo) {
                if (Util.isTrue(5, 100)) {
                    ItemMap itemMap = new ItemMap(mob.zone, 380, 1, x, yEnd, player.id, player);
                    list.add(itemMap);
                }
            }
        }
        ...
        if (MapService.gI().isMapCold(player.zone.map)) {// drop item cold
            if (Util.isTrue(1, 200)) {
                ItemMap itemMap = new ItemMap(mob.zone, 1992, 1, x, yEnd, player.id, player);
                list.add(itemMap);
            }
            if (player.setClothes.godClothes) {
                if (Util.isTrue(1, 100)) {
                    ItemMap itemMap = new ItemMap(mob.zone, Util.nextInt(663, 667), 1, x, yEnd, player.id, player);
                    list.add(itemMap);
                }
            }
        ...
        if (mapid == ConstMap.LANH_DIA_BANG_HOI) {// map bang
...
//                                player.clanMember.memberPoint++;
                        ItemMap itemMap = new ItemMap(mob.zone, ConstItem.CAPSULE_BANG_HOI, 1, x, yEnd, player.id, player);
                        list.add(itemMap);
        ...
        if (player.setClothes.SetHuyDiet >= 5) {
            if (mapid == 155) {
                if (Util.isTrue(10, 100)) {
                    ItemMap itemMap = new ItemMap(mob.zone, Util.nextInt(ConstItem.MANH_AO, ConstItem.MANH_GANG_TAY), 1, x, yEnd, player.id, player);
                    list.add(itemMap);
                }
            }
        }
        ...
        if (mobReward != null) {
            ...
            if (itemSize > 0) {
                ...
                if (Util.isTrue(1, 1000)) {
                    ItemMap itemMap = new ItemMap(mob.zone, 18, 1, x, yEnd, player.id, player);
                    list.add(itemMap);
                }
                if (Util.isTrue(5, 1000)) {
                    ItemMap itemMap = new ItemMap(mob.zone, 19, 1, x, yEnd, player.id, player);
                    list.add(itemMap);
                }
                if (Util.isTrue(1, 100)) {
                    ItemMap itemMap = new ItemMap(mob.zone, 20, 1, x, yEnd, player.id, player);
                    list.add(itemMap);
                }
                if (inMap) {
                    if (ir.forAllGender
                        ...
                        int ratio;
                        if (ir.tempId >= 441 && ir.tempId <= 447) {
//                            System.out.println(ir.typeRatio);
                            ratio = 10;
                        } else {
                            ratio = ir.ratio;
//                            System.out.println("id: " + ir.tempId + "\nratio: " + ir.ratio
//                                    + "\ntype ratio: " + ir.typeRatio);
                        }
                        if (Util.isTrue(ratio, ir.typeRatio)) {

                            ItemMap itemMap = new ItemMap(mob.zone, ir.tempId, 1, x,
                                    yEnd, player.id, player);
                            //init option
...
//                    rd.add(40, 568 /*, "DDijt nhau au au"*/);
                    ...
                    rd.add(40, 1955 /*, "DDijt nhau au au"*/);
                    for (int i = 0; i < 3; i++) {
                        int itemID = rd.next();
                        ItemMap itemMap = new ItemMap(mob.zone, itemID, 1, x + Util.nextInt(-50, 50), yEnd, player.id, player);
                        list.add(itemMap);
                    }
                    for (int i = 0; i < 10; i++) {
                        ItemReward gr = mobReward.goldRewards.get(Util.nextInt(0, goldSize - 1));
                        if (Util.isTrue(gr.ratio, gr.typeRatio)) {
                            ItemMap itemMap = new ItemMap(mob.zone, gr.tempId, 1, x + Util.nextInt(-50, 50), yEnd, player.id, player);
                            initQuantityGold(itemMap);
                            list.add(itemMap);
                        }
                    }
                }
//                if (MapService.gI().ismapsk(player.zone.map)) {// drop item cold
//                    if (Util.isTrue(1, 100)) {
//                        ItemMap itemMap = new ItemMap(mob.zone, 541, 1, x, yEnd, player.id, player);
//                        list.add(itemMap);
//                    }
//                }

                if (!player.isBoss && !player.isPet) {
                    if (player.inventory.itemsBody.get(8) != null && player.inventory.itemsBody.get(8).template != null && player.inventory.itemsBody.get(8).template.id == 1148) {

                        if (Util.isTrue(1, 100)) {
                            ItemMap itemMap = new ItemMap(mob.zone, 1287, 1, x, yEnd, player.id, player);
                            list.add(itemMap);
...
//                if (inMap) { cai nay de nghich =)))
//                    if (ir.forAllGender
//                            || ItemService.gI().getTemplate(ir.tempId).gender == player.gender
//                            || ItemService.gI().getTemplate(ir.tempId).gender > 2) {
////                        if (Util.isTrue(ir.ratio, ir.typeRatio)) {
//                        int slItem = Util.nextInt(10, 20);
//                        for (int i = 0; i < slItem; i++) {
//                            ItemMap itemMap = new ItemMap(mob.zone, ir.tempId, 1, 10 * i + x,
//                                    yEnd, player.id, player);
//                            switch (itemMap.itemTemplate.type) {
//                                case 0:
//                                case 1:
...
//                    ItemReward gr = mobReward.goldRewards.get(Util.nextInt(0, goldSize - 1));
//                    ItemMap itemMap = new ItemMap(mob.zone, gr.tempId, 1, x, yEnd, player.id, player);
//                    initQuantityGold(itemMap);
//                    list.add(itemMap);
//                }
//                if (MapService.gI().isMapBanDoKhoBau(mapid)) {
//                    if (Util.isTrue(1, 14)) {
//                        ItemMap itemMap = new ItemMap(mob.zone, 708, 1, x, yEnd, player.id, player);
