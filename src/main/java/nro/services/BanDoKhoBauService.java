...
public class BanDoKhoBauService {
    ...
    public void openBanDoKhoBau(Player player, byte level) {
        int turn = EventTurnManager.ManageCallDragon(ConstAction.GET_BY_ID, player.id);
        if (turn <= 0) {
            Service.getInstance().sendThongBao(player, "Bạn chỉ có thể đi bản đồ kho báu 1 ngày 5 lần");
            return;
        }
        if (level == 110) {
            if (player.clan != null && player.clan.banDoKhoBau == null) {
            ...
            Service.getInstance().sendThongBao(player, "Bạn chỉ được đi cấp độ 110");
        }
    }
}
