...
public class ClanService {
    ...
    public Clan getClanById(long plId, int id) throws Exception {
        return getClanById(plId, 0, Manager.getNumClan(), id);
    }

    private Clan getClanById(long plId, int l, int r, int id) throws Exception {
        if (l <= r) {
            int m = (l + r) / 2;
            Clan clan = null;
//            if (m >= 0 && m < Manager.CLANS.size()) {
//                removeClanPlayer((int) plId);
//            }
            ...
            return getClanById(plId, l, r, id);
        } else {
...
//        }
    ...
    private void sendInviteClan(Player player, int playerId) {
        ...
        if (pl != null && player.clan != null) {
            ...
            try {
                msg = new Message(-57);
                msg.writer().writeUTF(player.name + " mời bạn vào bang " + Util.RemoveTagSize(player.clan.name));
    ...
    private void acceptJoinClan(Player player, int clanId) {
        try {
            ...
            if (player.clan == null) {
                Clan clan = getClanById(player.id, clanId);
                ...
                if (pl != null) {
                    if (pl.clan == null) {
                        if (clan.getCurrMembers() < clan.maxMember) {
                            ...
                            Service.getInstance().sendThongBao(pl, "Bạn vừa được nhận vào bang " + Util.RemoveTagSize(clan.name));
//                            checkDoneTaskJoinClan(clan);
    ...
    private void cancelAskJoinClan(Player player, int clanMessageId) {
        ...
        if (clan != null && clan.isLeader(player)) {
            ...
            if (cmg != null) {
                ...
                if (newMember != null) {
                    Service.getInstance().sendThongBao(newMember, "Bang hội " + Util.RemoveTagSize(clan.name) + " đã từ chối bạn vào bang");
                }
                clan.sendMyClanForAllMember();
            }
        }
    ...
    private void createClan(Player player, byte imgId, String name) {
        if (player.clan == null) {
            ...
            if (flagBag != null) {
                ...
                Clan clan = new Clan();
                clan.imgId = imgId;
                clan.name = Util.RemoveTagSize(name);
    ...
    public void sendRemoveClan(Player player) {
        ...
        try {
            ...
            Log.error(ClanService.class, e, "Lỗi send my clan " + Util.RemoveTagSize(player.clan.name) + " - " + player.clan.id);
        }
    }
    ...
    public void sendMyClan(Player player) {
        ...
        try {
            ...
            Log.error(ClanService.class, e, "Lỗi send my clan " + Util.RemoveTagSize(clan.name) + " - " + clan.id);
        }
    }
        ...
        try {
            for (Clan clan : Manager.CLANS) {
                try {
                    ...
                    Log.error(Clan.class, e, "Có lỗi khi update clan " + Util.RemoveTagSize(clan.name));
                }
            }
        } catch (Exception e) {
