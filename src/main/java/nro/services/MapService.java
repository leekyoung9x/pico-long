...
//    }
    ...
    public List<Zone> getMapCapsule(Player pl) {
        ...
        // ekko thêm map thung lũng đá và capsule bay
        addListMapCapsule(pl, list, getZoneJoinByMapIdAndZoneId(pl, 212, 0));
        return list;
    }
    ...
    public boolean ismap212(int mapId) {
        return mapId == 212;
    }
    ...
    public static boolean isMapDiaNguc(int mapId) {
        return mapId == 208;
    }
    ...
    public boolean ismapsk(Map map) {
        int mapId = map.mapId;
        return mapId >= 122 && mapId <= 124;
    }

    public boolean isMapThungLungDa(int mapId) {
        return mapId == 212;
    }
    ...
    public boolean isMapCoTieuDoiSatThu(Zone zoneJoin) {
        if (zoneJoin != null && TaskService.gI().haveTDSTInZone(zoneJoin.getBosses())) {
            return true;
        } else {
            return false;
        }
