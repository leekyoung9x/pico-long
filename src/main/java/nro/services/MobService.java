        ...
        if (!player.isPet) {
            if (player.charms.tdThuHut > System.currentTimeMillis()) {
                for (ItemMap item : items) {
                    if (item.itemTemplate.id != 590 && item.itemTemplate.id != ConstItem.BINH_NUOC) {
        ...
        switch (mob.tempId) {
            ...
            case ConstMob.QUY_DAT:
                if (TaskService.gI().getIdTask(player) == ConstTask.TASK_2_0) {
                    itemMap = new ItemMap(mob.zone, 73, 1, mob.location.x, mob.location.y, player.id, player);
                }
                break;
//            case 88:
//            case 89:
//                if (Util.isTrue(1, 100)) {
//                    itemMap = new ItemMap(mob.zone, 1098, 1, mob.location.x, mob.location.y, player.id, player);
//                }
//                if (Util.isTrue(1, 100)) {
//                    itemMap = new ItemMap(mob.zone, 2031, 1, mob.location.x, mob.location.y, player.id, player);
//                }
//                if (Util.isTrue(1, 2000)) {
//                    if (player.nPoint.isX2ItemReward) {
//                        ArrietyDrop.DropItemSetKichHoat(player, player.gender, Util.getOne(1, 2), mob.location.x, mob.location.y);
//                    } else {
//                        ArrietyDrop.DropItemSetKichHoat(player, player.gender, 1, mob.location.x, mob.location.y);
//                    }
//                }
//                break;
            }
