...
import nro.utils.SpecialOptionUtil;
...
public class InventoryService {
    ...
    public boolean addItemBag(Player player, Item item, int maxQuantity) {
        ...
        switch (item.template.type) {
            ...
            case 10:
                player.inventory.addGem(item.quantity);
                Service.getInstance().sendMoney(player);
                return true;
            case 34:
                int oldRuby = player.inventory.ruby;
                player.inventory.addRuby(item.quantity);
                // ekko ghi log add ruby
                Manager.addPlayerRubyHistory(player.id, oldRuby, player.inventory.ruby, "InventoryService-addItemBag");
                Service.getInstance().sendMoney(player);
                return true;
        }
            ...
            for (Item it : items) {
                if (it.isNotNullItem() && it.template.id == item.template.id) {
                    if (item.template.id != 457 && item.template.id != 590
                        ...
                        // ekko gán lại chỉ số param thấp hơn của ki
                        if(it.template.id == ConstItem.SAO_PHA_LE_SIEU_CAP_KI) {
                            for (ItemOption io : it.itemOptions) {
                                if (io.optionTemplate.id == 103) {
                                    for(ItemOption io2 : item.itemOptions) {
                                        if(io2.optionTemplate.id == 103) {
                                            if(io2.param < io.param) {
                                                io.param = io2.param;
                                            }
                                        }
                                    }
    ...
    private Item putItemBody(Player player, Item item) {
        ...
        try {
            if (item.isNotNullItem()) {
                // ekko
                // bỏ sử dụng danh hiệu vs type = 99
                // bỏ sử dụng Aether stone vs type = 76
                // Đóng ô trang bị Chân mệnh vs type = 97
                // 29.9.24 cho dùng danh hiệu vs type = 99
                if (type >= 0 && type <= 5 || type == 32 || type == 11 || type == 23 || type == 24
                        || type == 72|| type == 76 || type == 21 || type == 69 || type == 99 || type == 97) {
                    if (item.template.gender == player.gender || item.template.gender == 3) {
                        ...
                        if (forPet) {
                            ...
                            if (item.template.type == 99) {
                                ...
                                Service.getInstance().sendTitle(player);//chanvong
                            }
                        } else {
...
//        SpecialOptionUtil.MakeOptionThayTro(player, item, true);
        if (item.isNotNullItem()) {
    ...
    public void itemBodyToBag(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
            ...
            Service.getInstance().removeTitle(player);//chanvong
            if (index == 12) {
                if (item.isNotNullItem()) {
                    Service.getInstance().removeEffPlayer(player, item.template.part);
                }
            }
        }
    }
...
//            Util.RemoveOptionThayTro(player, player.pet, item, false);
            if (item.isNotNullItem()) {
    ...
    public void itemBoxToBodyOrBag(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
            ...
            if (!done) {
                if (addItemBag(player, item, 1_500_000)) {
                    if (item.quantity == 0) {
                        Item sItem = ItemService.gI().createItemNull();
    ...
    public void itemBagToBox(Player player, int index) {
        ...
        Item item = player.inventory.itemsBag.get(index);
        if (item.isNotNullItem()) {
            if (addItemBox(player, item, 1_500_000)) {
                if (item.quantity == 0) {
                    Item sItem = ItemService.gI().createItemNull();
...
//                if (it.isNotNullItem() && it.template.id == item.template.id) {
    ...
    public Item findVeTangRuby(Player player) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 2079)) {
                return item;
            }
        }
        return null;
    ...
    public Item findItemEat(Player player) {
        for (Item it : player.inventory.itemsBag) {
            if (it.isNotNullItem() && it.isItemThucAn() && it.quantity >= 10) {
                return it;
            }
        }
        return null;
    ...
    // ekko hàm tìm thức ăn theo số lượng
    public Item findMealQuantity(Player player, int quantity) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id >= 663 && item.template.id <= 667) && item.quantity >= quantity) {
                return item;
            }
        }
        return null;
    ...
    // ekko hàm tìm thức ăn theo số lượng
    public boolean checkSoLuongThucAn(Player player, int quantity) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id >= 663 && item.template.id <= 667) && item.quantity >= quantity) {
                return true;
            }
    ...
    // ekko check player mặc ít nhất 1 đồ thiên sứ
    public boolean checkMacDoThienSu(Player player) {
        if (player.setClothes.setThienSu >= 1) {
            return true;
        }
        return false;
    }
    ...
    // ekko tìm item theo option id
    public Item findItemBagByOptionID(Player player, int optionID) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem()) {
                for (ItemOption io : item.itemOptions) {
                    if (io.optionTemplate.id == optionID) {
    ...
    // ekko tìm item theo option id
    public Item findItemBodyByOptionID(Player player, int optionID) {
        for (Item item : player.inventory.itemsBody) {
            if (item.isNotNullItem()) {
                for (ItemOption io : item.itemOptions) {
                    if (io.optionTemplate.id == optionID) {
    ...
    public void fixChiSo(Player player) {
        List<Item> itemBag = player.inventory.itemsBag; // check item bag
        ReSetOptionForItem(itemBag);
        List<Item> itembody = player.inventory.itemsBody; // check item bag
        ReSetOptionForItem(itembody);
        List<Item> itembox = player.inventory.itemsBox; // check item bag
        ReSetOptionForItem(itembox);
        if (player.pet != null) {
            List<Item> itempet = player.pet.inventory.itemsBody; // check item bag
            ReSetOptionForItem(itempet);
        }
    }

    private static void ReSetOptionForItem(List<Item> items) {
