...
public class ChangeMapService {
    ...
    public void changeMapNonYard(Player player, int mapid, int x, int y) {
        Zone zone = MapService.gI().getMapCanJoin(player, mapid);
        ChangeMapService.gI().changeMap(player, zone, -1, -1, x, y, TELEPORT_YARDRAT);
    }
    ...
    public void changeMap(Player pl, Zone zoneJoin, int mapId, int zoneId, int x, int y, byte typeSpace) {
        if (pl.zone.map.mapId == 129) {
            pl.isMapPk = true;
        }

        if (mapId != 129 && pl.isMapPk) {
            pl.isMapPk = false;
            System.out.println(pl.isMapPk);
...
//                    Service.getInstance().sendThongBaoOK(pl, "Sức mạnh vượt ngưỡng 200 tỷ không thể đến map này");
//                    return;
//                }
//
//                if (MapService.gI().ismap212(pl.zone.map.mapId)) {
//                    Zone zone = MapService.gI().getZoneJoinByMapIdAndZoneId(pl, 0, 0);
//                    if (zone != null) {
//                        pl.location.x = 500;
//                        pl.location.y = zone.map.yPhysicInTop(500, 100);
//                        MapService.gI().goToMap(pl, zone);
...
//            }
            ...
            if (nextMapIsCold && pl != null && pl.playerTask != null && pl.playerTask.taskMain != null && pl.playerTask.taskMain.id < 24) {
                Service.getInstance().sendThongBaoOK(pl, "Bạn chưa hoàn thành Nhiệm vụ chạm trán Rôbốt sát thủ lần 2");
                return;
            }

            // ekko thung lũng đá
            boolean currMapIsThungLungDa = MapService.gI().isMapThungLungDa(pl.zone.map.mapId);
            boolean nextMapIsThungLungDa = MapService.gI().isMapThungLungDa(zoneJoin.map.mapId);
            ...
            // ekko thung lũng đá
            if (currMapIsThungLungDa != nextMapIsThungLungDa) {
                if (!currMapIsThungLungDa && nextMapIsThungLungDa) {
                    Service.getInstance().sendThongBao(pl, "Bạn đã đến Thung lũng đá");
                    if(pl.nPoint != null && !pl.nPoint.isKhongNong) {
                        Service.getInstance().sendThongBao(pl, "Bạn đang ở sa mạc nóng bức nên chỉ số bị giảm một nữa");
                    }
                } else {
                    Service.getInstance().sendThongBao(pl, "Bạn đã rời thung lũng đá");
                    if(pl.nPoint != null && !pl.nPoint.isKhongNong) {
            ...
            // map doanh trại thì tính toán lại chỉ số (/ 5 dame, hp, ki)
            if (currMapIsDoanhTrai != nextMapIsDoanhTrai) {
                Service.getInstance().point(pl);
                Service.getInstance().Send_Info_NV(pl);
            }
...
//            }
    ...
    public Zone checkMapCanJoin(Player player, Zone zoneJoin) {
        ...
        // vào thời gian từ 14h -> 19h hàng ngày khi hoàn thành nhiệm vụ tđst sẽ không thể vào khu có boss
        ConstMap.now = LocalTime.now();
        if(zoneJoin != null && MapService.gI().isMapCoTieuDoiSatThu(zoneJoin)) {
            // hoàn thành nhiệm vụ TDST thì k cho vô
            if (ConstMap.now.isAfter(ConstMap.startTDST) && ConstMap.now.isBefore(ConstMap.endTDST) && player.playerTask.taskMain.id > 20) {
                return null;
            }
        }

        // ekko đạt 200 tỉ sức mạnh thì bị sút khỏi map
//        if (zoneJoin != null && zoneJoin.map != null && MapService.gI().ismap212(zoneJoin.map.mapId)) {
//            // ekko người chơi đạt 200 tỉ sức mạnh thì bị sút khỏi map
//            if (player.nPoint != null && player.nPoint.power >= 200_000_000_000L) {
//                return null;
//            }
//        }
