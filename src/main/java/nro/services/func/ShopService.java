...
public class ShopService {
    ...
    private static final int COST_GOLD_BAR = 2_000_000_000;
    ...
    private ItemShop getItemShop(int shopId, int tempId) {
        ...
        switch (shopId) {
            ...
            case ConstNpc.SHOP_CHIEN_LUC:
                shop = getShop(ConstNpc.DAISHINKAN, 0, -1);
                break;
            case ConstNpc.SHOP_VIP:
                shop = getShop(ConstNpc.ONGLAO, 0, -1);
                break;
            case ConstNpc.SHOP_VIP2:
                shop = getShop(ConstNpc.DUONG_TANG, 0, -1);
                break;
            case ConstNpc.SHOP_BUNMA_LINH_THU:
                shop = getShop(ConstNpc.BUNMA_TL, 1, -1);
                break;
            case ConstNpc.SHOP_WHIS_DO_THIEN_SU_VIP:
                shop = getShop(ConstNpc.WHIS, 1, -1);
                break;
            case ConstNpc.SHOP_LTN_SPEC:
                shop = getShop(ConstNpc.LY_TIEU_NUONG, 1, -1);
                break;
            case ConstNpc.SHOP_BILL_SPEC:
                shop = getShop(ConstNpc.BILL, 1, -1);
                break;
            case ConstNpc.CUA_HANG_KY_GUI:
                shop = getShop(ConstNpc.CAY_NEU, 0, -1);
                break;
        }
        if (shop != null) {
    ...
    public Shop getShop(int npcId, int order, int gender) {
...
//        player.iDMark.setShopId(shopId);
    ...
    public void openShopType3(Player player, Shop shop, int shopId) {
        player.iDMark.setShopId(shopId);
        if (shop != null) {
            Message msg;
            try {
...
//                        player.nPoint.tiemNang -= power;
                ...
                openShopLearnSkill(player, ConstNpc.URON, ConstNpc.SHOP_LEARN_SKILL, 0, player.gender);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void buyItemShopNormal(Player player, ItemShop is) {
        ...
        if (is != null) {
            ...
            // ekko validate shop thiên sứ vip
            else if (player.iDMark.getShopId() == ConstNpc.SHOP_WHIS_DO_THIEN_SU_VIP) {
                boolean isValidThucAn = InventoryService.gI().checkSoLuongThucAn(player, 999);
                boolean isValidTS = InventoryService.gI().checkMacDoThienSu(player);
                if (!isValidThucAn) {
                    Service.getInstance().sendThongBao(player, "Yêu cầu có 999 thức ăn");
                    return;
                }
                if (!isValidTS) {
                    Service.getInstance().sendThongBao(player, "Bạn phải mang trên người một set Thiên Sứ");
                    return;
                }
            }
            ...
            if (is.temp.id == 379 && player.playerTask.taskMain.id < 24) {
                Service.getInstance().sendThongBao(player, "Bạn phải hoàn thành nhiệm vụ pick pock cink conk");
                return;
            }
            ...
            if (is.temp.id == 637) {// su
                boolean hasRequiredItem = false;
                Item requiredItem = InventoryService.gI().findItemBag(player, 636);// mai
                if (requiredItem != null) {
                    hasRequiredItem = true;
                }
                if (!hasRequiredItem) {
                    Service.getInstance().sendThongBao(player, "Hãy mua cải trang mai trước");
                    return;
                }
            }
            if (is.temp.id == 635) {// su
                boolean hasRequiredItem = false;
                Item requiredItem = InventoryService.gI().findItemBag(player, 637);// su
                if (requiredItem != null) {
                    hasRequiredItem = true;
                }
                if (!hasRequiredItem) {
                    Service.getInstance().sendThongBao(player, "Hãy mua cải trang su trước");
                    return;
                }
            }

            if (is.temp.id == 710) {//chun
                boolean hasRequiredItem = false;
                Item requiredItem = InventoryService.gI().findItemBag(player, 711);// quy lao
                if (requiredItem != null) {
                    hasRequiredItem = true;
                }
                if (!hasRequiredItem) {
                    Service.getInstance().sendThongBao(player, "Hãy mua cải trang Jacky Chun trước");
                    return;
                }
            }
            ...
            if (InventoryService.gI().getCountEmptyBag(player) > 0) {
                ...
                if (itemExchange >= 0) {
                    ...
                    } else if (itemExchange == 77 && player.inventory.getGem() >= is.costSpec) {
                        player.inventory.subGem(is.costSpec);
                ...
                switch (player.iDMark.getShopId()) {
                    ...
                    case ConstNpc.SHOP_GIAM_GIA:
                        ...
                        if (pGG != null) {
                            ...
                            if (item.template.id == 591) {
                                int random = Util.nextInt(1, 40);
                                item.itemOptions.add(new ItemOption(77, random));

                                random = Util.nextInt(1, 40);
                                item.itemOptions.add(new ItemOption(103, random));

                                random = Util.nextInt(1, 40);
                                item.itemOptions.add(new ItemOption(50, random));

                                if (Util.isTrue(1, ConstRatio.PER100)) {
                                    item.itemOptions.add(new ItemOption(33, random));
                                } else {
                                    item.itemOptions.add(new ItemOption(30, random));
                                }
                            }
                    ...
                    case ConstNpc.SHOP_WHIS_THIEN_SU:
                        ...
                        break;
                    case ConstNpc.SHOP_WHIS_DO_THIEN_SU_VIP:
                        int quantityMeal = 999;
                        Item mealTS = InventoryService.gI().findMealQuantity(player, quantityMeal);
                        if (mealTS != null) {
                            Item itemTS = ItemService.gI().createItemFromItemShop(is);
                            int paramTS = 0;
                            if (Util.isTrue(2, 10)) {
                                paramTS = Util.nextInt(10, 15);
                            } else if (Util.isTrue(3, 10)) {
                                paramTS = Util.nextInt(0, 10);
                            }
                            for (ItemOption io : itemTS.itemOptions) {
                                ...
                                switch (optId) {
                                    ...
                                    case 23: //ki k
                                    case 14: //crit
                                        io.param += ((long) io.param * paramTS / 100);
                                        break;
                                }
                            }
                            InventoryService.gI().subQuantityItemsBag(player, mealTS, quantityMeal);
                            InventoryService.gI().addItemBag(player, itemTS, 99);
                            InventoryService.gI().sendItemBags(player);
                            Service.getInstance().sendThongBao(player, "Đổi thành công " + is.temp.name);
                        } else {
                            Service.getInstance().sendThongBao(player, "Yêu cầu có 999 thức ăn");
                            ...
                            if (Util.isTrue(20, 100)) {
                                if (Util.isTrue(1, 2)) {
                                    switch (player.gender) {
                                        ...
                                        case (byte) ConstPlayer.XAYDA: {
                                            itemShop.itemOptions.add(new ItemOption(134, 0));
                                            itemShop.itemOptions.add(new ItemOption(251, 3));
                                            break;
                                        }
                                        case (byte) ConstPlayer.NAMEC: {
                                            itemShop.itemOptions.add(new ItemOption(130, 0));
                                            itemShop.itemOptions.add(new ItemOption(142, 100));
                                            break;
                                        }
                                    }
                                } else {
    ...
    public void showConfirmSellItem(Player pl, int where, int index) {
        ...
        if (item.isNotNullItem()) {
            ...
            Message7(pl, item, where, index, goldReceive);
...
//            }
    ...
    public void Message7(Player pl, Item item, int where, int index, int goldReceive) {
        Message msg = new Message(7);
        try {
            msg.writer().writeByte(where);
            msg.writer().writeShort(index);
    ...
    public void sellItem(Player pl, int where, int index) {
        ...
        if (item != null && item.isNotNullItem()) {
            ...
            if (item.template.id == ConstItem.RUONG_QUA_DAC_BIET) {
                ...
                int quantity = item.quantity * 5000;
                coint.quantity = quantity;
                InventoryService.gI().addItemBag(pl, coint, 0);
                InventoryService.gI().subQuantityItemsBag(pl, item, item.quantity);
                InventoryService.gI().sendItemBags(pl);
            ...
            if (pl.inventory.gold + goldReceive <= pl.inventory.getGoldLimit()) {
                ...
                pl.inventory.addGem(gem);
                int oldRuby = pl.inventory.ruby;
                pl.inventory.addRuby(ruby);
                // ekko ghi log add ruby
                Manager.addPlayerRubyHistory(pl.id, oldRuby, pl.inventory.ruby, "ShopService-sellItem");
                pl.inventory.gold += goldReceive;
                pl.playerTask.achivements.get(ConstAchive.TRUM_NHAT_VE_CHAI).count++;
                PlayerService.gI().sendInfoHpMpMoney(pl);
                String nr = "Đã bán " + item.template.name + " thu được " + Util.numberToMoney(goldReceive) + " vàng";
                String content = "Đã bán " + item.template.name + " thu được " + Util.numberToMoney(gem) + " vàng";
                Service.getInstance().sendThongBao(pl, (thanlinh == null ? nr : content));
            } else {
