...
//            }
        ...
        if (VangorNgoc == 0) {
            if (!players.stream().anyMatch(game -> game.id == player.id && game.point == point)) {
                ...
                data.point = point;
                players.add(data);
                player.inventory.addGem((int) -giaSo_Gem);
    ...
    public String strFinish(int id) {
        ...
        for (ConSoMayManData g : players) {
            if (id == g.id && g.point == result && g.conSoMayManNgoc == 1) {
                ...
                player.inventory.addRuby((int) MiniGame.gI().MiniGame_S1.money);
                // ekko ghi log add ruby
                Manager.addPlayerRubyHistory(player.id, oldRuby, player.inventory.ruby, "ConSoMayMan-strFinish");
