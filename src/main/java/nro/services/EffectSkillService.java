...
//        player.effectSkill.timeAnTroi = timeAnTroi;
    ...
    public void setMaPhongBe(Player player, long lastTime, int time) {
        player.effectSkill.lastTimeMaPhongBe = lastTime;
        player.effectSkill.timeMaPhongBe = time;
        player.effectSkill.isMaPhongBe = true;
    }
    ...
    public void removeMaPhongBe(Player pLayer) {
        pLayer.effectSkill.isMaPhongBe = false;
        Service.getInstance().Send_Caitrang(pLayer);
    }
    ...
    public void sendMobTransform(Player player, Mob mob, int timeSocola, short iconId) {
        Message msg;
        try {
            msg = new Message(-112);
            msg.writer().writeByte(1);
            msg.writer().writeByte(mob.id); //mob id
            msg.writer().writeShort(iconId); //icon socola
            Service.getInstance().sendMessAllPlayerInMap(player, msg);
            msg.cleanup();
            mob.effectSkill.setSocola(System.currentTimeMillis(), timeSocola);
        } catch (Exception e) {
    ...
    public void sendMobToBinh2(Player player, Mob mob, int timeBinh) {
        Message msg;
        try {
            msg = new Message(-112);
            msg.writer().writeByte(1);
            msg.writer().writeByte(mob.id);
            msg.writer().writeShort(11166);
            Service.getInstance().sendMessAllPlayerInMap(player, msg);
            msg.cleanup();
    ...
    public void setBinh2(Player player, long lastTimeBinh, int timeBinh) {
        player.effectSkill.lastTimeBinh = lastTimeBinh;
        player.effectSkill.timeBinh = timeBinh;
        player.effectSkill.isBinh2 = true;
